...
String returnKeyOnValue(Map baseLFMaps, Object searchValue){
    for(String key : baseLFMaps.keySet()){
        if(baseLFMaps.get(key).equals(searchValue)){
    ...
    for (Map.Entry lfEntry : lfMaps.entrySet()) {
        for (Map.Entry baseLFEntry : baseLFMaps.entrySet()) {
            if ((baseLFEntry.getKey() == lfEntry.getKey()) && (lfEntry.getValue() != baseLFEntry.value)) {
                ...
                myLFToReplaceMaps.put(lfEntry.getKey().replace("[", "").replace("]", ""),returnKeyOnValue(baseLFMaps,baseLFEntry.value))
...
void mergeLogicalFields(String pathToNewLfFile, File baseLfFile, String pathToMergeFile) {
    ...
    Map myTempmergeLfMap = new HashMap<>()
    ...
    baseLfJson.each {
        ...
        if (lfItem != null) {
            ...
            for (int j = 0; j < lfItem.size; j++) {
                ...
                lfCurrentFields=lfItem[j].fields
                lfCurrentMpName=lfItem[j].mpName
                ...
                for (int i = 0; i < it.value.size; i++) {
                    ...
                    //запоминаем элемент для мержа
                    itemForAddInMerge = lfItem[j]
                    //  !!! можно в если добавить на одинаковость MPName  !!!
//                    if (lfCurrentFields==lfBaseCurrentFields &&!lfMyMaps.containsKey(lfCurrentMpName)){
                    if (lfCurrentFields==lfBaseCurrentFields && lfCurrentMpName==lfBaseCurrentMpName){
                        tempLf=(String)lfCurrentMpName
                        tempbaseLf=(String)lfBaseCurrentMpName
                        if (tempLf!=tempbaseLf){
                            lfMyMaps.put(tempLf.replace("[", "").replace("]", ""), tempbaseLf.replace("[", "").replace("]", ""))
                        }
                        isExistField=true
                        break
                    }
                ...
                // Добавляем в список элекменты из chain, которых нет в quote
                if (!isExistField){
                    itemsForMergeList.add(lfItem[j])
                    println(lfItem[j])
                }
            }
        ...
        // temp add all items from base-lf  for merge
        for (int j = 0; j < it.value.size; j++) {
            itemsForMergeList.add(it.value[j])
            myTempmergeLfMap.put(it.key, itemsForMergeList)
        }
...
// UNCOMENT!!!!!!!!!!!!!!!!
//printMap(lfToReplaceMaps, "NEW MERGE FILES  --> LOGICAL FIELDS FOR REPLACE IN RULE AND LF:")



printMap(lfMyMaps, "NEW MERGE FILES  --> LOGICAL FIELDS FOR REPLACE IN RULE AND LF:")
//temp
//replacementFieldsInFile(outDir.path + newRuleFileName, outDir.path + newRuleFileName, lfMyMaps)


//replace fields in rule after  merge !!!!!!!!!!!!!1UNCOMENT!!!!!!!!!1
